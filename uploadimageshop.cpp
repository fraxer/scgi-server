class UploadImageShopModel : public Model {
    public:
        string addContent(const string& post_id, const string& content_type, const string& json) {

            query("SELECT `id`, COUNT(`id`) as `count` FROM `product_content` WHERE (`id_parent` = " + post_id + " AND `type` = 0) LIMIT 1");
        ...
        string reservePlace(const string& user_id) {
            query("INSERT INTO `product` (`id`, `visibility`, `id_adder`, `type_adder`, `param`, `desc`, `date`) VALUES (NULL, '0', '" + user_id + "', '0', '', '', CURRENT_TIMESTAMP)");

            string post_id = to_string(mysql_insert_id(mysql));

            mysql_free_result(res);

            query("INSERT INTO `product_content` (`id`, `visibility`, `type`, `id_adder`, `type_adder`, `id_parent`, `param`, `desc`, `hash`, `date`) "
                "VALUES "
            ...
            query("UPDATE `user_uploads` SET `id_parent_shop_content` = " + post_id + " WHERE `id_user` = " + user_id);
        ...
        bool hasPlace(const string& user_id) {
            query(
                "SELECT "
                    "`id_parent_shop_content`, "
                    "(SELECT COUNT(`c`.`id`) FROM `product_content` as `c` WHERE `c`.`id_parent` = `uu`.`id_parent_shop_content` AND `c`.`type` = 0 LIMIT 1) "
                "FROM "
                    "`user_uploads` as `uu`"
...
class UploadImageShopController: public Controller {
    public:
        UploadImageShopController() {
            fillRights();
        }
        ...
        string getResForPOST(map<const char*, const char*, cmp_str>* parms, UploadImageShopModel& model, User& user) {
...
//                resizeImage(url_s, url_mob, 150);
        ...
        string run(map<const char*, const char*, cmp_str>* parms) {
            ...
            if(!hasRight(user.role, "uploadImageShop"))
                return "{\"status\":\"0\"}";
...
//            addHeader("Content-Type:application/json; charset=UTF-8");
        ...
        void fillRights() {
            vector<string> admin_operations;
                           admin_operations.push_back("uploadImageShop");

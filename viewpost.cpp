class ViewPostModel : public Model {
    public:
        bool getData(const string& id, const string& type, const string& start, const string& count, const User& user) {
            ...
            query(
                ...
                "WHERE "
                    "c.id = " + id + " AND (c.visibility = 1 " + str2 + ") "
                //"ORDER BY c.id ASC"
            );
        ...
        string& getJSON(const string& id, const string& type, const string& start, const string& count, const User& user) {
            ...
            while(res != NULL && (row = mysql_fetch_row(res)) != NULL)
            {
                 ...
                 str += "{"
                     ...
                     "\"login\":\"" + string(row[3]) + "\","
                     "\"avatar\":\"" + string(row[4]) + "\","
                     "\"amount_comments\":" + string(row[5]) + ","
                     "\"amount_like\":" + string(row[6]) + ","
                     "\"like\":" + (user_is_set ? row[7] : "0") + ""
                 "}" + _s;
...
class ViewPostController: public Controller {
    public:
